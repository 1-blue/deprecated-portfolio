 ...
 */
export const setDetailsHeight = ($detailsList: HTMLDetailsElement[]) => {
 ...
 */
export const imageCarouselSetting = ($imgWrapper: HTMLElement) => {
  // 이미지들을 가진 이미지 컨테이너
  ...
  // 이미지 dot 버튼 컨테이너
  const $dotButtonContainer = $imgWrapper.querySelector(
    ".img-dot-btn-container"
  );
  if (!($dotButtonContainer instanceof HTMLElement)) return;
  // 이미지 컨테이너 내부의 이미지들
  const $imgs = [...$imgContainer.childNodes].filter(
    ($img): $img is HTMLElement => $img instanceof HTMLElement
  );
  // 이미지 개수 / 이동 거리
  ...
  const imgMoveBtnClickEvent = (e: MouseEvent) => {
    e.stopPropagation();
    if (isExcute) return;
    ...
    $imgContainer.style.transform = `translateX(${
      direction === -1 ? rDistance : lDistance
    }%)`;

    // 현재 이미지 동기화
    currentTaget += direction === -1 ? 1 : -1;
    if (currentTaget >= imgCount) currentTaget = 0;
    else if (currentTaget < 0) currentTaget = imgCount - 1;
  ...
  const imgTransitionEndEvent = () => {
    if (!isExcuteTransitionEnd) return;
    if (!($imgContainer.firstElementChild instanceof HTMLElement)) return;
    if (!($imgContainer.lastElementChild instanceof HTMLElement)) return;
  ...
  const imgDotBtnClickEvent = (e: MouseEvent) => {
      ...
      ([{ isIntersecting }]) => {
        ...
        if ((!direction && !isIntersecting) || (direction && isIntersecting)) {
          $$anchors.forEach(($$anchor) =>
            $$anchor.classList.remove("activate")
          );
...
export const scrollProgressBarSetting = () => {
  ...
  if (!($scrollProgressBar instanceof HTMLElement)) return;
