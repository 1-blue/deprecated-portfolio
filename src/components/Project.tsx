...
const Project = ({
  project: { name, description, links, skills, thumbnails, date, logoColor },
}: ProjectProps) => {
  return (
    <li
      className="bg-indigo-600 px-10 py-6 rounded-md text-white shadow-xl"
      {...useScrollZoom({})}
    >
      <h3 className="font-bold text-4xl text-center">{name}</h3>
        ...
        <div className="space-y-8">
          ...
          <ul className="flex justify-evenly">
            ...
            <CustomLink href={links.deploy}>
              <Icon shape="logo" className={`text-[${logoColor}]`} />
            </CustomLink>
          </ul>
        </div>
      </div>
